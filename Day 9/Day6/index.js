...
<body>
    <div class="header">
        ...
        <form>
        <input type="text" placeholder="Search..." id="inp" onkeyup="search()">
        </form>
        </div>
        <div class="tabs">
...
const finalHtml = htmlTemplate
  ...
  .replace("_script_",scriptTemplate);
    ...
    if (urlId > 0 && urlId <= 30) {
      res.end(`
        ...
        <div class="q-card-body" style="padding: 10px;">
            <a href="product?id=${
              urlId - 1
            }" class="q-btn" style="padding: 8px 12px; background-color: #007bff; color: white; text-decoration: none; border-radius: 5px; margin-right: 10px;">Prev</a>
            <a href="product?id=${
              +urlId + 1
            }" class="q-btn" style="padding: 8px 12px; background-color: #28a745; color: white; text-decoration: none; border-radius: 5px;">Next</a>
        </div>
    </div>
...
server.listen(3500, "localhost", () => {
  console.log("working on http://localhost:3500");
